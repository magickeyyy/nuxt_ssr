    ...
    <div class="middle">
        <div class="module" id="level0">
            ...
            <div class="goods tuancan" v-for="(v, i) in skuList" :key="i">
                ...
                <div class="contant">
                    <Row v-for="(item,index) in v" :key="index" :gutter="20">
                        <Col span="15">
                            <h3>{{item.name}}</h3>
                            <ul class="tags" style="color:#999;">
                                <li
                                    v-if="item.minimumOrderQuantity&&item.minimumOrderQuantity>0"
                                >起订量：{{item.minimumOrderQuantity}}</li>
                                <li
        ...
        <div class="module" id="level2">
            ...
            <div class="notice" v-if="detail.reserveKnows">
                <h3>注意事项</h3>
                <div class="pre"><p v-for="(v,i) in detail.reserveKnows" :key="i">{{v}}</p></div>
            </div>
            <div class="notice">
                <h3>退款规则</h3>
                <Table
...
export default {
    ...
    created() {
        this.skuList = this.classifySku(this.detail.skuList);
        if (
            this.detail.skuList &&
    ...
    methods: {
        booking(v) {
            // 能否预定是canCancel，isCancel不知道什么玩意儿
            let cancelPolicy = {};
            ...
            this.$axios({...API_PLAY.resource, data }).then(res => {
                if (res.success) {
                    ...
                    this.$router.push({ name: 'play-expOrder', query: { type: RESOURCE_TYPE_TO_SERVER[2].value}});
                }
            });
        },
        classifySku(list) {
