    ...
    <Form ref="form"
        ...
        <div style="width:100%;display:flex;padding-top:28px;">
            <FormItem prop="destination"
                ...
                <Cascader placeholder="请选择目的地国家、城市"
                          class="his destination"
                          v-model="form.destination"
                          :data="destinationList"
                          :load-data="loadCity"
                          :clearable="false"
                          @on-change="changeDestination"></Cascader>
            </FormItem>
            <BookHotel ref="date"
                       :scene="'hotel'"
                       :date="[form.checkInDate,form.checkOutDate]"
                       trigger="custom"
                       :onChange="changeDate">
                <div class="bookDate"
                     @click="changeVisible">
            ...
            <FormItem prop="room"
                ...
                <InputNumber class="roomNum"
                             :min="1"
                             v-model="form.bookingQuantity"
                             placeholder="房间数"></InputNumber>
            </FormItem>
            <FormItem prop="member"
                      style="margin:0;">
                <Dropdown ref="sss"
                          class="his memberInfo"
                          trigger="custom"
                          :visible="visible"
                          @on-clickoutside="visible=false">
                    <div :class="[{'active':visible},'member']"
                         @click="visible=!visible;">
                        <div :class="{'active':form.adultsQuantity||form.childQuantity}">{{form.adultsQuantity}}成人&nbsp;·&nbsp;{{form.childQuantity}}儿童</div>
                        ...
                        <div class="setMember">
                            <div class="top">
                                <div>
                                    <p>每间入住成人</p>
                                    <div class="change">
                                        <div :class="['jian',{disabled:form.adultsQuantity<=1}]"
                                             @click="addQuantity(-1, 'adultsQuantity')"><i class="iconfont iconjianshao"></i></div>
                                        <p>{{form.adultsQuantity}}成人</p>
                                        <div :class="['jia',{disabled:form.adultsQuantity>=9}]"
                                             @click="addQuantity(1, 'adultsQuantity')"><i class="iconfont iconzengjia"></i></div>
                                    </div>
                                </div>
                                <div>
                                    <p>每间入住儿童</p>
                                    <div class="change">
                                        <div :class="['jian',{disabled:form.childQuantity<=0}]"
                                             @click="addQuantity(-1, 'childQuantity')"><i class="iconfont iconjianshao"></i></div>
                                        <p>{{form.childQuantity}}儿童</p>
                                        <div :class="['jia',{disabled:form.childQuantity>=9}]"
                                             @click="addQuantity(1, 'childQuantity')"><i class="iconfont iconzengjia"></i></div>
                                    </div>
                                </div>
                            </div>
                            <div class="middle"
                                 v-show="form.childQuantity">
                                <p>请选择入住时儿童年龄</p>
                                <Select v-for="(v,i) in form.childAge"
                                        :key="i"
                                        v-model="form.childAge[i]">
                                    <Option v-for="(item,index) in ageList"
                                            :value="index"
        ...
        <div style="width:100%;display:flex;padding-top:20px;">
            ...
            <FormItem style="margin:0;padding-right:20px;">
                <Select v-model="form.nationality"
                        class="his nation"
                        placeholder="请选择国籍">
                    <Option v-for="(v,i) in nationList"
                            :value="v.twoCharCode"
                            :key="i">{{ v.locationNameChn }}</Option>
                </Select>
            </FormItem>
            <FormItem class="seeOnlyGroupRooms">
                <Checkbox :class="['his', {checked: form.seeOnlyGroupRooms}]"
                          v-model="form.seeOnlyGroupRooms">只看团房</Checkbox>
            </FormItem>
            <Button type="primary"
                    class="searchBtn"
...
export default {
    ...
    computed: {
        ...
        nightNum() {
            ...
            if (this.form.checkInDate && this.form.checkOutDate) {
                return (
                    (new Date(this.form.checkOutDate).valueOf() -
                        new Date(this.form.checkInDate).valueOf()) /
                    86400000
                )
            } else {
                return 0
    ...
    mounted() {
        if (!this.hotel_search || JSON.stringify(this.hotel_search) === '{}') {
            ...
            this.getCountryList()
            this.getNationList()
            this.SET_SEARCH(this.mixin_m_SStorage('get', 'hotel_search') || {})
        }
    },
    methods: {
        ...
        addQuantity(n, key) {
            ...
            if (
                this.form[key] + n > 9 ||
                this.form[key] + n < (key === 'adultsQuantity' ? 1 : 0)
            )
                return
        ...
        getCountryList() {
            this.$axios({ ...API_LOCATION.allCountry }).then(res => {
                if (res.success) {
        ...
        getNationList() {
            this.$axios({ ...API_LOCATION.nationList }).then(res => {
                if (res.success) {
            ...
            this.$axios({
                ...
                if (res.success) {
                    ...
                    let selectedCity = citys.find(
                        v => v.value === this.form.destination[1]
                    )
                    if (selectedCity) {
                        // 必须重设一次，不然不回显
...
.hisSearchHotel {
    ...
    .bookDate {
        ...
        background: #fff url('~assets/icon/icon_calendar_def.png') no-repeat
            12px center;
        cursor: pointer;
        &:hover {
    ...
    .destination {
        @{deep}.ivu-input-wrapper {
            ...
            .ivu-input {
                padding-right: 40px;
                background: url('~assets/icon/icon_local_def.png') no-repeat
                    10px center;
            }
            .ivu-input:focus,
            .ivu-input:hover {
        ...
        .box {
            ...
            .history {
                ...
                .title {
                    ...
                    font-weight: bold;
                    div:hover {
                        background: url('~assets/icon/icon_trash_active.png')
                            no-repeat center;
                    }
                    div {
                        width: 40px;
                        height: 76px;
                        cursor: pointer;
                        background: url('~assets/icon/icon_trash_def.png')
                            no-repeat center;
                    }
                }
                ul {
    ...
    .hotelName {
        @{deep}.ivu-input {
            background: url('~assets/icon/icon_hotel_def_999.png') no-repeat
                10px center;
    ...
    @{deep}.nation {
        ...
        .ivu-select-selection {
            ...
            > div {
                ...
                line-height: 54px;
                padding-left: 40px;
                background: url('~assets/icon/icon_nation_def.png') no-repeat
                    10px center;
            }
    ...
    @{deep}.searchInput {
        .ivu-input {
            width: 360px;
            background: url('~assets/icon/icon_local_def.png') no-repeat 10px
                center;
        }
        .ivu-input:focus {
            border-bottom-color: #e9047b;
        ...
        .default {
            ...
            .title {
                ...
                div {
                    ...
                    height: 76px;
                    cursor: pointer;
                    background: url('~assets/icon/icon_trash_def.png') no-repeat
                        center;
                }
            }
            .options {
