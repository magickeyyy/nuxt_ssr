...
<template>
    <div class="detail">
        <div class="route">
            ...
            <nuxt-link v-if="detail" :to="{name:'hotel-detail',query:{hotelId:$nuxt.$route.query.hotelId}}" exact style="color:#e9047b">{{detail.nameChn}}预定</nuxt-link>
        </div>
        <div class="hotelInfo" v-if="detail">
            <div class="left">
                <div class="name">
                    <h3>{{detail.nameChn}}</h3>
                    <Rate disabled allow-half :value="detail.star*1" class="his" />
                </div>
                <p>{{detail.nameEng}}</p>
                <div class="local">{{detail.addressEng}}</div>
                <div class="img" v-if="detail.imageList">
                    <img :src="detail.imageList[0].imgUrl" alt="加载失败" v-noimg="'1120-430'" />
                    <div @click="modal = true" v-if="detail.imageList[0].imgUrl!=='ssxxcc'">共{{detail.imageList.length}}张照片</div>
                </div>
            </div>
            <div class="right">
                <div class="rate">
            ...
            <Modal v-model="modal" footer-hide :width="980" v-if="detail.imageList">
                <div class="showImgs">
                    <Carousel v-model="imgOrder" loop>
                        <CarouselItem v-for="(v,i) in detail.imageList" :key="i" style="width:792px;">
                            <img :src="v.imgUrl" alt="加载失败" v-noimg="'1120-430'" />
                        </CarouselItem>
                    </Carousel>
                    <ul>
                        <li v-for="(v,i) in detail.imageList" :key="i" :class="{'active': imgOrder===i}">
                            <img :src="v.imgUrl" alt="加载失败" v-noimg="'480-276'" />
                        </li>
                    </ul>
                </div>
            </Modal>
        ...
        <div class="hotelDetail">
            <Anchor :affix="false" class="anchor" @on-select="activeLevel">
                <AnchorLink v-for="(v,i) in anchor" :href="`?${urlData}#level${i}`" :title="v" :class="{'active':currentLevel===i}" :key="i" />
            </Anchor>
            <!-- 团房 -->
            <div class="teamRoom" v-if="login&&HiseasRoomRate.RoomList.length>0">
                <div class="title"></div>
                <div class="roomInfo">
                    <div class="item" v-for="(item,index) in HiseasRoomRate.RoomList" :key="index">
                        <div class="name">
                            <div>{{item.RoomTypeName}}</div>
                        </div>
                        <div class="bedList" v-for="(v,i) in [item.RateInfoList[0]]" :key="i">
                            <div class="left">
                                <div>
                                    适合住人数&emsp;
                            ...
                            <div class="btn">
                                <Button type="primary" @click="bookingTeam(item,HiseasRoomRate.SupplierId)">预定</Button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <!-- 好巧房源:散客酒店 -->
            <h3 id="level0" class="title">全部房型</h3>
            <div>
                <div class="total" v-for="(item,index) in HaoQiaoRoomRate.RoomList" :key="index">
                    <ul class="thead">
                    ...
                    <div v-for="(v1,i1) in item.RateInfoList" :key="i1">
                        <div class="tbody">
                            <div class="left w220">
                                <div class="img">
                                    <img :src="v1.ImageList[0]||'error'" alt="" v-noimg="''">
                                </div>
                                <div class="name">{{v1.RateNameChn}}</div>
                                <p>{{v1.RateNameEng}}</p>
                            </div>
                            <ul class="right">
                                <li v-for="(v2,i2) in v1.BedList" :key="i2">
                                    <div class="w188">
                                        <p style="font-weight:bold;">{{v2.Name}}</p>
                                        <div>{{v2.Desc}}</div>
                                    </div>
                                    <div class="w140" :style="{color:v1.Breakfast.NumBreakfasts==0?'#999999':''}">{{v1.Breakfast.BreakfastDesc}}</div>
                                    <div class="w208">
                                        <p v-for="(v3,i3) in v1.CancelPolicy" :key="i3">{{v3 | tranCancelPolicy}}</p>
                                    ...
                                    <div>
                                        <Button type="primary" @click="booking(v1,HaoQiaoRoomRate.SessionId,HaoQiaoRoomRate.SupplierId)">预定</Button>
                                    </div>
                                </li>
                            </ul>
                        </div>
                ...
                <div class="login" v-if="!login">
                    <Button type="primary" @click="tohome">请登录以查看价格</Button>
                </div>
                <div v-if="login&&!HaoQiaoRoomRate.RoomList.length===0" class="login">暂无数据</div>
            </div>
            <h3 id="level1" class="title">酒店介绍</h3>
            <div id="hotelInfo" v-if="detail">
                <p>{{detail.introductionChn}}</p>
            </div>
            <h3 id="level2" class="title">酒店设施</h3>
            <div id="hotelFacility">
                <div>设施</div>
                <ul>
                    <li v-for="(v,i) in facility" :key="i">{{v.nameChn}}</li>
                </ul>
            </div>
            <h3 id="level3" class="title">条款</h3>
            <div id="hotelClause">
...
export default {
    ...
    filters: {
        ...
        tranCancelPolicy(v) {
            // 把每个房型的每条取消政策转成文本
            if(!v) return '不支持退改';
            return new Date(v.cancelPolicyDate * 1).toLocaleDateString() + '前退改收费' + (v.cancelPolicyCancelCharge * 1).toFixed(2) + '元';
        }
    },
    computed: {
        showRoomRateList() {
            let arr = this.roomRateList;
            if (arr.length && arr.find(v => v && v.RoomList.length > 0)) return true;
            else return false;
        },
        facility() {
            // 设施
            if (this.detail && this.detail.facilities) {
                return this.detail.facilities.filter(v => v.facilityDes === '有');
            } else {
                return [{}];
            }
        },
        ...
        checkIn() {
            ...
            const reg = /^\d+:\d+$/g;
            if (this.detail && this.detail.facilities) {
                return this.detail.facilities.find(v => v.nameEng === 'Check-in') || {};
            } else {
                return {};
            }
        },
        checkOut() {
            ...
            const reg = /^\d+:\d+$/g;
            if (this.detail && this.detail.facilities) {
                return this.detail.facilities.find(v => v.nameEng === 'Check-out') || {};
            } else {
                return {};
            }
        },
    ...
    created() {
        ...
        if (this.mixin_m_SStorage('has', 'hotel_search')) {
            this.getRoomDetail().then(res => {
                if (res) {
    ...
    methods: {
        ...mapMutations('hotel', ['SET_ORDERPRE']),
        bookingTeam(item, supplierId) {
            // 游海团组酒店预定
            let hotelId = this.$route.query.hotelId * 1;
            let search = JSON.parse(sessionStorage.getItem('hotel_search'));
            let rateCode = [];
            ...
            this.$axios.post(api_hotel.bookingPre, params, { custom: { token: true } }).then(res => {
                if (res.success) {
                    let result = res.data;
                    result.cancelPolicy = this.cancelPolicy2(JSON.parse(result.cancelPolicy));
                    this.SET_ORDERPRE({
                        type: 'YH',
                        pre: {
                            ...result,
        ...
        booking(item, sessionId, supplierId) {
            ...
            this.$axios.post(api_hotel.bookingPre, params, { custom: { token: true } }).then(res => {
                if (res.success) {
                    let result = res.data;
                    result.cancelPolicy = this.cancelPolicy(JSON.parse(result.cancelPolicy));
                    this.SET_ORDERPRE({
                        type: 'HQ',
                        pre: {
                            ...res.data,
        ...
        getAllotment() {
            ...
            let arr = [];
            if (search) {
                let { bookingQuantity, adultsQuantity, childAge, childQuantity } = { ...search };
                for (let i = 0; i < bookingQuantity; i++) {
                    ...
                    for (let j = 0; j < adultsQuantity + childQuantity; j++) {
                        ...
                        if (j > adultsQuantity - 1) {
                            arr[i][j].adult = false;
                            arr[i][j]['childAge'] = childAge[j - adultsQuantity] * 1;
                        }
                    }
                }
            }
        ...
        cancelPolicy(arr) {
            ...
            return arr.map(v => {
                v.cancelPolicyDate = this.mixin_m_formatDate(v.cancelPolicyDate * 1);
                v['str'] = `${v.cancelPolicyDate}前取消收费${v.cancelPolicyCancelCharge}`;
                return v;
            });
        },
        // 团组取消政策
        cancelPolicy2(arr) {
            ...
            return arr.map(v => {
                v.cancelPolicyDate = this.mixin_m_formatDate(v.cancelPolicyDate * 1);
                v['str'] = `${v.cancelPolicyDate}后取消收费${v.cancelPolicyCancelCharge}`;
                return v;
            });
        },
        getRoomDetail() {
            ...
            return this.$axios.post(api_hotel.hotelDetail, { hotelId }).then(res => {
                if (res.success) {
                    if(!res.data) this.$router.go(-1);
                    if (!res.data.imageList || res.data.imageList.length < 1) {
                        res.data.imageList = [{ imgUrl: 'ssxxcc' }];
                    }
                    res.data.commentScore = (res.data.commentScore / 10).toFixed(1);
                    this.detail = res.data;

                    window.setRes({
                        name1: res.data.nameChn,
                        name2: resType['HOTEL'],
                        sp: res.data.addressChn || res.data.addressEng,
                        img: '',
                    });

                    return res.data.nameChn;
                } else {
        ...
        getRoomList() {
            ...
            if (!this.mixin_m_SStorage('has', 'token')) {
                ...
                let hotelId = this.$nuxt.$route.query.hotelId * 1;
                let search = this.mixin_m_SStorage('get', 'hotel_search');
                let data = {
                ...
                this.$axios.post(api_hotel.roomRateList, data, { custom: { token: true } }).then(res => {
                    if (res.success) {
                        if (res.data.HaoQiaoRoomRate && res.data.HaoQiaoRoomRate.RoomList.length > 0) {
                            this.HaoQiaoRoomRate = res.data.HaoQiaoRoomRate;
                        }
                        if (res.data.HiseasRoomRate && res.data.HiseasRoomRate.RoomList.length > 0) {
    ...
    #hotelFacility {
        ...
        > ul {
            ...
            li {
                width: 190px;
                padding-left: 20px;
                background: url('~assets/icon/icon_correct_active.png') no-repeat 0;
                color: #6c6c6c;
            }
        }
    }
...
@{deep}.ivu-modal-content {
    ...
    .ivu-modal-body {
        ...
        .showImgs {
            height: 500px;
            background-image: linear-gradient(180deg, #ea037c 6px, #fff 6px, #fff 100%);
